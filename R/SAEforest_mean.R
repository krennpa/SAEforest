SAEforest_mean <- function(Y, X, dName, smp_data, pop_data, MSE = "none", aggData =FALSE,
  ...
  if(MSE == "nonparametric"){
    MSE_estims <- MSE_SAEforest_mean(Y=Y, X = X, dName = dName, smp_data = smp_data,
  ...
  if(aggData == TRUE){
    ...
    if(MSE == "nonparametric"){
      ...
      MSE_estims <- MSE_SAEforest_aggOOB(Y=Y, X = X, mod = mean_preds, smp_data = smp_data,
